...
- status: superseded-by-S7-signed-off-relation-runtime
...
> Current status after S7 signoff: 本文件是 S7 开工前的边界记录。S7a-S7f 已完成并签收，正式结论见 `docs/v1.5/acceptance/S7-stage7-acceptance.md`。本文件保留为历史 preflight，用于解释为什么不在原始 `timeWindow + calculatedFields` 通道中开放二次聚合 / 二次窗口；当前有效口径以 `docs/v1.5/S7-CTE-timeWindow-capability-recap.md` 为准。
...
同时，Stage 7 的前置抽象应从"如何在 timeWindow DSL 里拼二次能力"调整为"如何把任意 QueryPlan 转成稳定 view/relation"。该方向已在后续 S7a-S7f 中落地：Java runtime 已支持 `CompiledRelation` 外层 read-only / aggregate / window，Python 负责 snapshot consumer / mirror；但 `timeWindow + calculatedFields` 原通道仍保持二次聚合 / 二次窗口 fail-closed。
...
以下门槛已由 S7a-S7f 关闭，保留在此用于后续同类扩展复核：
...
- [x] Stage 7 当前状态确认：S7a-S7f 已完成并签收；S8 未启动。
- [x] 候选能力业务动机记录完成。
- [x] 当前 fail-closed 边界记录完成。
- [x] `Plan -> Stable View/Relation` 前置抽象已升级为 S7a formal contract draft。
- [ ] 如未来启动 S8 或 named / recursive CTE，按具体候选项新开独立 follow-up。
...
- 是否可进入实现：否；S7 已关闭，未来扩展需新开 S8 或独立契约。
