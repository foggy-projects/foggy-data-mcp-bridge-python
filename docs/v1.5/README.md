...
- `P1-timeWindow-Python-parity-progress.md` — 跟踪 Java 已签收 `timeWindow` 能力在 Python 引擎的后续对齐。当前完成到 S3c DTO / MCP payload passthrough + validator + rolling/cumulative expansion IR + rolling/ytd/mtd 两层 SQL path + value/range lowering + yoy/mom/wow comparative self-join SQL path；MySQL8 / 多方言实跑矩阵和覆盖审计仍未完成；不改变本 README 中计算字段编译器三阶段的签收结论。
