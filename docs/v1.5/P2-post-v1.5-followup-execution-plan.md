...
| F-7 datasource identity contract | Python + Java + Odoo Pro if consumed | Python complete / Java pending | Python 已选择 `ModelInfoProvider.get_datasource_id`；Java 镜像仍待做 |
...
| Java compose compiler | `foggy-data-mcp-bridge-wt-dev-compose` | pending | 镜像 Python 的 datasource identity 判断与错误码行为 |
...
- status: Python-complete / Java-pending
...
- Python implementation quality gate: `docs/v1.5/quality/S1-S2-post-v1.5-followup-implementation-quality.md`, decision `ready-with-risks` because Java mirror remains pending.
- Java `@Disabled` / placeholder 测试仍待转 regular pass。
...
1. Complete Stage 1 Java mirror if the next goal is full two-engine F-7 closure.
2. Stage 3: Java/Python normalized SQL golden diff harness.
3. Stage 4: SQL Server timeWindow real DB matrix when environment exists.
4. Stage 5: Phase 4 AST optional only if AST default flip becomes a concrete goal.
5. Stage 6: Future Java contract expansions only after Java publishes contract + fixtures.

## Current Decision

- Python Stage 1 and Stage 2 are implemented and verified.
- Full two-engine F-7 closure still requires Java datasource identity mirror.
- Stage 3 can start after deciding whether Java F-7 mirror should be completed first.
