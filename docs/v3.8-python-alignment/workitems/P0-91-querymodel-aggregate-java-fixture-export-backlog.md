...
no-column / aliased-key / tenant guard no-leak slice. P0-112 adds the
Python-side contract-only fixture and manifest gate for the planned
`querymodel-aggregate-join-4` O615 export without marking those cases as
Java-exported.
...
3. Implement Java export and Python actual replay for the P0-112
   `querymodel-aggregate-join-4` O615 contract before implementing more
   runtime behavior in the positive O615 graph path.
