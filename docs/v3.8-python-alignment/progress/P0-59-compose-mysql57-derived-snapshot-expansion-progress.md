# P0-59 Progress - Compose MySQL 5.7 Derived Snapshot Expansion
...
- Added Java exporter case
  `derived-filter-order-limit-mysql57-subquery` in
  `JavaComposeSnapshotTest`.
- Refreshed
  `tests/fixtures/java_compose_snapshot_parity.json` from the Java exporter.
- Updated Python coverage inventory assertions to require at least `25` cases,
  at least `21` successful cases, full strict success coverage, and no missing
  `mysql/derived` success cell.
- Updated the Java snapshot parity manifest with MySQL 5.7 derived subquery
  fallback evidence.
...
- `.venv/bin/python scripts/summarize-compose-snapshot-coverage.py`
  - Result: `25` cases, `21/21` strict successful cases.
