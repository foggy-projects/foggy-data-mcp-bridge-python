...
- P0-79+ now records the aggregate-join continuation sequence as completed
  through P0-95, with P0-87 v2 snapshot/replay active, the
  ...
  outer-only predicates, and public diagnostics, and P0-95 opens bounded
  `orderBy` and `returnTotal` support for the narrow SQLite aggregate relation
  path.
...
6. Keep the QueryModel aggregate join P0 lane engine-neutral. P0-79 through
   P0-95 now close the first narrow SQLite boundary plus governance/API
   ...
   public diagnostics, and bounded `orderBy` / `returnTotal`; follow-up P0/P1
   work should review replay-only v3 cases before expanding behavior.
...
1. Implement QueryModel aggregate join in Python if required:
   ...
   - P0-82 through P0-85 cover the first SQLite boundary, and P0-87 through
     ...
     behavior, public diagnostics, and bounded `orderBy` / `returnTotal`.
     Remaining P2 scope is external dialects, broader metadata shape evidence,
     multi-relation support, composite/dimension path runtime implementation,
     richer request-stage contracts, O615-shaped business regressions, and
...
3. **Aggregate join is partially proven for the narrow Python SQLite path; broad parity remains open.**
   P0-79 through P0-95 now provide carrier, guarded attachment, SQLite SQL/live
   result, governance, V3 metadata, diagnostics, Java v3 replay, null-check
   outer-only predicates, unsafe runtime-filter refusal, and bounded
   `orderBy` / `returnTotal`. Remaining aggregate gaps are external dialects,
   composite/dimension path runtime behavior, multi-relation support, O615-style
   no-column/alias/tenant regressions, post stages, `groupBy`/`having`,
