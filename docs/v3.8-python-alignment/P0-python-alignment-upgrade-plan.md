...
| Time window / relative date | Java supports timeWindow in query paths; pivot forbids direct timeWindow and routes time intelligence through calculated fields. P0-66 refreshes the current Java SQL snapshot for timeWindow happy cases and records the current `wow-week-happy` `salesDate$week` catalog/model generation drift explicitly. | Python has `time_window.py`, Java parity catalog fixture, refreshed Java SQL snapshot replay, SQLite execution, real DB matrix tests, and v1.15 acceptance for timeWindow. P0-66 requires 8 Java-success SQL snapshots plus one documented Java generation error, and replays every Java-success case through Python validate mode. | Core timeWindow validate/SQLite coverage is active. Remaining gaps are the Java catalog/model drift for `wow-week-happy`, full normalized SQL diff for multi-CTE timeWindow SQL, live DB/result parity beyond existing SQLite execution tests, and pivot+timeWindow rejection stability. | Medium | P1 | Keep P0-66 snapshot replay, Java catalog replay, and SQLite execution tests active. Next: either fix/export the `wow-week-happy` drift or add live result parity for the current 8-case Java success set. Keep pivot+timeWindow refusal tests in P0 smoke set. |
...
3. Refresh timeWindow relative-date and pivot/domain-transport edge behavior.
   ...
   success set and records the remaining `wow-week-happy` Java drift; pivot
   and domain-transport edge behavior remain separate follow-ups.
