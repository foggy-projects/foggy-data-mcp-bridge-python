...
| Inline formula / calculated fields / alias behavior | Java includes formula compiler parity, predefined formula fixes, inline formula/calculated fields, alias behavior, v3.0 semantic money scale, and 9.2 formula follow-ups. | Python has formula compiler/capability tests, formula field extraction, semantic service formula compiler, timeWindow/calculatedFields history, v1.16 same-stage alias fix, P0-33 explicit HAVING aggregate-alias strictness, and P0-35 explicit HAVING aggregate alias field-collision refusal. Current pytest has formula parity snapshot/catalog failures outside the recent alias boundary fixes. | Core aggregate alias boundaries are tighter, but broader formula snapshot parity and post-aggregate calculated-field staging remain open. Current parity snapshot tests and Java formula/money-scale follow-ups still need a bounded replay/update lane before product-facing claims. | High | P0/P1 | P0: repair or regenerate Java formula snapshot catalog evidence without changing engine behavior. P1: implement bounded formula gaps only after current dirty changes are understood; include alias-in-slice/order/group tests, explicit HAVING alias collision refusals, post-aggregate staging refusals, and semantic scale golden cases. |
...
3. **Aggregate join is not proven in Python; semantic scale live evidence remains optional.**
   ...
   field-collision refusal is active in P0-35, and the remaining
   semantic-scale gaps are namespace opt-out and live DB/result evidence.
