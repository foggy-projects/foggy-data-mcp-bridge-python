...
| Script/runtime tool | Java has `dataset.compose_script`, `DataSetResult`, `ComposedDataSetResult`, `QueryPlan.execute/to_sql`, script parity tests, and tool-level MCP entry. P0-4 exports tool markers, runtime globals, basic result shape, preview SQL capture, and security fail-closed cases. P0-21 adds execute-mode rows envelope replay for `return { plans: dsl(...) }`. P0-22 adds a dedicated MCP host-misconfig error payload snapshot for resolver-null. P0-23 adds remote authority-binding principal-mismatch error payload replay. | Python has `ComposeScriptTool`, ContextVar runtime bundle, `execute_sql`, plan execution, capability registry/library loader, JS fixture parity tests, and MCP binding tests. P0-4 replay validates shared tool markers and runtime cases; P0-21 replay validates execute-mode `plans` row-list shape; P0-22/P0-23 replay structured host/remote error fields and forbidden leakage markers. README still says `dataset.compose_query` is pending. | Neutral snapshot lane is active through tool markers, runtime globals, preview SQL capture, fail-closed security parameters, execute rows envelope, resolver-null host-misconfig, and remote principal mismatch. Remaining gaps are full DataSetResult/ComposedDataSetResult method and composed-result shape parity, missing context/header bridge payloads, resolver exception behavior, remote missing-binding error-code/phase decision, capability allow/deny snapshots, and a decision on Python's extra fsscript globals. | Medium | P0/P1 | Keep P0-4/P0-21/P0-22/P0-23 replay active; extend it with full DataSetResult/composed-result, remaining error payload matrix cases after contract decisions, and capability fail-closed fixtures before changing production runtime behavior. |
