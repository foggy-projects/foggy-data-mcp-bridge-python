...
- P0-87 expands the Java/Python fixture to 19 cases for fieldAccess,
  ...
  narrow SQLite aggregate relation path.
...
- P0-79+ now records the aggregate-join continuation sequence as completed
  ...
  slice complete, and P0-88 contract ready.
...
1. Implement QueryModel aggregate join in Python if required:
   ...
   - P0-82 through P0-85 cover the first SQLite boundary, and P0-87 adds the
     first runtime fieldAccess/system_slice slice. Remaining P2 scope is
     external dialects, broad metadata/API DTO exposure, calculatedFields over
     aggregate outputs, multi-relation support, and production TMS DB evidence.
