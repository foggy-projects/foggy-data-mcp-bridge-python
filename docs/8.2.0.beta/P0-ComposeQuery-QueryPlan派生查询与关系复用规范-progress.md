...
| M7 | MCP `script` 工具入口（body 仅 `script` 文本，ToolExecutionContext → ComposeQueryContext） | **`python-ready-to-execute`** · r2 | 2026-04-23 | — | — | **Python 开工提示词 `M7-ScriptTool-Python-execution-prompt.md` r2（吸收 plan-evaluator 3 blocker + 6 非阻塞项）**· 目标 ≥60 tests · 新增 `runtime/` 子包（`context_bridge` / `script_runtime` / `plan_execution` / `errors` + `ComposeRuntimeBundle` ContextVar）+ `compose.script` MCP 工具 + `QueryPlan.execute/to_sql` 替换占位为真实实现 · **零新错误码命名空间**（复用 M1/M3/M4/M5/M6 code 家族 + 新增明文 tag `host-misconfig` / `internal-error` 用于非 compose 失败） · **Step 0 前置**：`SemanticQueryService.execute_sql(sql, params, *, route_model)` 新公共方法（raw-SQL 执行入口） · `ToolExecutionContext → ComposeQueryContext` 桥两档（嵌入 host 塞 Principal / header 路径 `Authorization / X-User-Id / X-Tenant-Id / X-Roles / X-Dept-Id / X-Policy-Snapshot-Id / X-Namespace / X-Trace-Id`） · evaluator 可见面锁定：承认 `_setup_builtins` 既有 17 个 builtin + 追加 `{from, dsl}`，`ALLOWED_SCRIPT_GLOBALS` frozenset 硬断言；不建虚构 `COMPOSE_SCRIPT_GLOBALS` 单独 ctx · host 基础设施走 `contextvars.ContextVar[ComposeRuntimeBundle]`（不用 `ComposeQueryContext.extensions`，ctx 语义不污染） · `QueryPlan.to_sql` 返回 `ComposedSql`（替换 M2 `SqlPreview` 占位；`SqlPreview` 类保留不破 M2 契约） · Java 侧提示词待 Python 落地后起草（ThreadLocal 等价 + Function<ToolExecutionContext, AuthorityResolver> factory 冻结） · 预估 3.0 – 3.5 PD（r1 估算 2.3 PD 偏低，吸收 Step 0 + evaluator 封装 + 异步执行桥接） |
