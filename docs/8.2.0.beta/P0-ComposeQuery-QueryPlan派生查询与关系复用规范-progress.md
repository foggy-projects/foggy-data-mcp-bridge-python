...
| M7 | MCP `script` 工具入口（body 仅 `script` 文本，ToolExecutionContext → ComposeQueryContext） | **`python-ready-to-execute`** | 2026-04-23 | — | — | **Python 开工提示词 `M7-ScriptTool-Python-execution-prompt.md` 已下发**（r1 · `ready-to-execute`）· 目标 ≥60 tests · 新增 `runtime/` 子包（`context_bridge` / `script_runtime` / `plan_execution` / `errors`）+ `compose.script` MCP 工具 + `QueryPlan.execute/to_sql` 替换占位为真实实现 · **零新错误码**（全部复用 M1/M3/M4/M5/M6 code 家族 + spec §错误模型 phase 映射） · `ToolExecutionContext → ComposeQueryContext` 桥两档（嵌入 host 塞 Principal / header 路径 `Authorization / X-User-Id / X-Tenant-Id / X-Roles / X-Dept-Id / X-Policy-Snapshot-Id / X-Namespace / X-Trace-Id`） · `COMPOSE_SCRIPT_GLOBALS = {"from": from_, "dsl": from_}`（仅两 global；Layer B `require / process / eval / globalThis` 禁） · `contextvars` 承载嵌套脚本 ctx 隔离 · Java 侧提示词待 Python 落地后起草 · 预估 2.3 PD |
