...
- columns 支持简单 `agg(field) as alias`，以及 `sum/avg/count(if(...)) as alias`
- `if(...)` 条件判断使用 `==`，多条件组合使用 `&&` / `||`
- Python 引擎会在编译阶段把 `if(...)` 统一降级为 SQL `CASE WHEN ... THEN ... ELSE ... END`
- DSL 不支持直接输出原始 `CASE WHEN` 片段，也不新增 `count_if / sum_if / avg_if` 契约
- 更复杂的表达式仍使用 calculatedFields
